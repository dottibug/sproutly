import { View, Text, StyleSheet, Pressable } from 'react-native';
import CategoryFilterIcon from './CategoryFilterIcon';
import Heading from '../ui/Heading';
import { colors } from '../../styles/theme';
import { CATEGORY_FILTERS, type CategoryFilter } from '../../state/filters/filterTypes';

const HEADING = 'Filter by Plant Type';

type CategoryFiltersProps = {
  readonly selectedFilters: Set<CategoryFilter>;
  readonly onToggleFilter: (category: CategoryFilter) => void;
};

// TODO: Finalize colors for selected/unselected (add those colors to the theme)

// Renders a list of filters with icons to filter catalog seeds by plant type (veggie, flower, fruit, herb). Must pass in the selected filters as a set, and a function to toggle the filters.
export default function CategoryFilters({ selectedFilters, onToggleFilter }: CategoryFiltersProps) {
  return (
    <View style={styles.plantTypeFiltersContainer}>
      <Heading size="small" uppercase>
        {HEADING}
      </Heading>
      <View style={styles.filtersContainer}>
        {CATEGORY_FILTERS.map((category) => {
          const selected = selectedFilters.has(category);
          return (
            <Pressable
              key={category}
              onPress={() => onToggleFilter(category)}
              style={[styles.filterContainer, selected && styles.selectedFilterContainer]}>
              <CategoryFilterIcon category={category} />
              <Text style={styles.filterText}>{category}</Text>
            </Pressable>
          );
        })}
      </View>
    </View>
  );
}

const styles = StyleSheet.create({
  plantTypeFiltersContainer: {
    borderBottomWidth: 1,
    borderColor: '#ccc',
    flexDirection: 'column',
    gap: 8,
    marginBottom: 12,
    paddingBottom: 4,
    paddingHorizontal: 16,
  },
  filtersContainer: {
    alignItems: 'center',
    flexDirection: 'row',
    justifyContent: 'space-between',
    marginBottom: 12,
  },
  filterContainer: {
    alignItems: 'center',
    borderColor: '#ccc',
    borderRadius: 9,
    borderWidth: 1,
    flexDirection: 'column',
    gap: 8,
    justifyContent: 'center',
    padding: 8,
    width: 84,
  },
  selectedFilterContainer: {
    backgroundColor: 'rgba(128, 0, 128, 0.1)',
    borderColor: 'purple',
  },
  filterText: {
    color: colors.primary,
    fontSize: 14,
    fontWeight: 'bold',
  },
});
