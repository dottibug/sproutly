...
import { buildUserSeedPhoto } from '../utils/photoUtils';
...
export async function insertPhoto(userId: string, userSeedId: string, imagePath: string): Promise<UserSeedPhoto> {
  ...
  return buildUserSeedPhoto(data.id, data.user_seed_id, data.user_id, data.image_url, data.created_at);
}
...
export async function fetchPhotosByUserSeedId(userSeedIds: string[]): Promise<UserSeedPhoto[]> {
  ...
  const photos = data?.map((photo) => buildUserSeedPhoto(photo.id, photo.user_seed_id, photo.user_id, photo.image_url, photo.created_at));
