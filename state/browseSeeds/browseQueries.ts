...
import { fetchPlantingActions } from '../userSeeds/queries/seedQueries';
...
async function fetchCatalogSeeds(): Promise<BrowseSeed[]> {
  const { data, error } = await supabase.from('seed_catalog').select(CATALOG_SELECT).order('name', { ascending: true });
